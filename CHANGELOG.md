...
- Optional note write log for diagnosing storage problems, off by default: enable it in the plugin settings ("Log note writes") or with `localStorage.setItem("inscriptum-write-log", "1")` in the Obsidian console — every note save then appends one JSON line to `.inscriptum-write-log.jsonl` at the vault root (time, trigger, path, sizes, result, duration). The setting persists; delete the localStorage key to drop the quick override.
