...
- Optional note write log for diagnosing storage problems: enable with `localStorage.setItem("inscriptum-write-log", "1")` in the Obsidian console, and every note save appends one JSON line to `.inscriptum-write-log.jsonl` at the vault root (time, trigger, path, sizes, result, duration). The flag survives app reloads; delete the key to turn it off.
