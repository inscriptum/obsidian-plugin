...
export const imageContainerNode = litView.node(function* (params: {
  ...
  // So that we connect a local property to a global key attribute to be sure that it's actual
  ...
  while (true) {
    ...
    const { src, text, subtext, error } = params.state ?? {
      text: "",
      subtext: "",
    };

    const isPlaceholder =
      (src == null || !isImageReadyByKey[params.key]) && error == null;
      ...
      <>
        {is(
          ...
          <ImageErrorNode
            ...
            text={error || ""}
            errorIconId={params.errorIconId}
