...
export function handleTouchStart(
  ...
  const circle =
    (overlay?.querySelector(
      ".texto-table__overlay_overlay-circle",
    ) as HTMLElement | null) ??
  ...
  if (circle) {
    ...
    runCellDrag(
      view,
      startEvent,
      Number(circle.dataset.cellPos) || null,
      CIRCLE_CLEARANCE,
    );
...
function dragToCellHead(
  ...
  return {
    anchor: $anchor,
    head: $head,
    topLeft: topLeftAnchor ?? $anchor.pos,
  };
}
...
function dispatchCellSelection(
  ...
  view.dispatch(
    view.state.tr.setSelection(selection).setMeta(tableEditingKey, selection),
  );
...
function redrawPicked(
  ...
  let rect = document.querySelector<HTMLElement>(
    ":scope > body > ." + PICKING_RECT_CLASS,
  );
  if (!anchor) {
    rect?.remove();
    return null;
...
function startLongPressCellSelection(
  ...
  function activate(): void {
    ...
    anchorDom = startCell.isConnected
      ? startCell
      : (document
