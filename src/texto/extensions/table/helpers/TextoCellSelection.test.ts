...
describe("handleCellSelection appendTransaction", () => {
  it("normalizes a plain CellSelection to TextoCellSelection", () => {
    ...
    const next = state.apply(
      state.tr.setSelection(CellSelection.create(doc, p1, p4)),
    );
  ...
  it("keeps an already-normalized selection stable", () => {
    ...
    const next = state.apply(
      state.tr.setSelection(new TextoCellSelection(doc.resolve(p1))),
    );
...
describe("resize handle decorations (mobile view)", () => {
  function decorations(
    ...
    selection: (doc: ProseMirrorNode) => TextSelection | CellSelection,
  ) {
    const doc = tableDoc();
    ...
    const draw = plugin.spec.props?.decorations as
  ...
  const caretInFirstCell = (doc: ProseMirrorNode) =>
    TextSelection.create(doc, cellPositions(doc)[0] + 1);
