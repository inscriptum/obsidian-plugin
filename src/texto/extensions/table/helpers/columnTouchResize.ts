 ...
 */
export function startColumnTouchResize(
  view: EditorView,
  ...
  const startWidth = currentColWidth(
    view,
    cellPos,
    cell.attrs as { colspan: number; colwidth?: number[] | null },
  );
  ...
  function move(event: TouchEvent) {
    ...
    lastWidth = Math.max(
      CELL_MIN_WIDTH,
      Math.round(startWidth + (touch.clientX - startX)),
    );

    if (tableDom?.firstElementChild instanceof HTMLElement) {
      updateColumns(
        table,
        tableDom.firstElementChild,
        tableDom,
        CELL_MIN_WIDTH,
        col,
        lastWidth,
...
function updateColumnWidth(state: EditorState, cellPos: number, width: number) {
  ...
  for (let row = 0; row < map.height; row += 1) {
    ...
    const colwidth = attrs.colwidth
      ...
      : new Array<number>(attrs.colspan).fill(0);
