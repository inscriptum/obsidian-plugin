...
export const nodeStatePluginKey = new PluginKey('nodeStatePlugin');
...
export function createNodeStatePlugin(typeName: Set<string>, editor: Editor, options: StateOptions) {
		...
		state: {
			init(_config, state) {
				...
				state.doc.nodesBetween(0, state.doc.nodeSize - 2, (node, startPos) => {
					...
					if (typeName.has(node.type.name) && node.attrs?.key == null) {
						if (node.attrs.data?.id == null) {
							tr.delete(pos, pos + node.nodeSize);

							offsetPos += node.nodeSize;
						} else {
							const key = `${String(Date.now())}_${pos}`;
							tr.setNodeMarkup(pos, node.type, {...node.attrs, key});
			...
			apply(this: Plugin, tr, set, oldState, _newState) {
				const newDecorationSet = mapSafetyNodeMarkup.call(editor, set, tr, (removedDeco) => {
					const node = getNodeByKey(oldState, removedDeco.spec.id);

					const {isChangeOrigin} = getTransactionsMetadata([tr]);

					if (node != null && isFunction(options.hooks?.onRemove)) {
						options.hooks.onRemove(node, removedDeco, {isLocalChange: !isChangeOrigin});
					}
				});

				let actions: NodeStatePluginAction | NodeStatePluginAction[] = tr.getMeta(this) ?? [];

				if (!Array.isArray(actions)) {
					actions = [actions];
				}

				return nodeStatePluginAction(options, tr, oldState, newDecorationSet, actions);
			},
		},
		props: {
			decorations(state) {
	...
	for (const action of actions) {
		if (action?.add != null) {
			...
			if (node != null && isFunction(options.hooks?.onAdd)) {
				options.hooks.onAdd(node, deco, {isLocalChange: !action.add.transactionsMeta.isChangeOrigin});
			}
			...
			decos.forEach((deco) => {
				const node = getNodeByKey(oldState, deco.spec.id);
...
function appendTransaction(
	...
	const nodeStatePlugin: DecorationSet = nodeStatePluginKey.getState(newState);

	for (const range of mergedRanges) {
		newState.doc.nodesBetween(range.from, Math.min(range.to, newState.doc.nodeSize), (node, pos) => {
			...
			if (node.attrs.key == null) {
				...
				const key = `${String(Date.now() + (nodeStatePlugin as any)['local'].length)}_${pos}`;
			...
			} else if (findPosByKey(newState, node.attrs.key) == null) {
				trMetaNodeState.add({
					add: {
						id: node.attrs.key,
						from: pos,
