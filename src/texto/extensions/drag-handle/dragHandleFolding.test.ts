...
const headingFolds = (view: Editor["view"]) =>
  Array.from(headingFoldingKey.getState(view.state)!.folded).sort(
    (a, b) => a - b,
  );
...
describe("dragging collapsed blocks", () => {
  it("a collapsed heading resolves to the whole section (heading + hidden body)", () => {
    ...
    view.dispatch(
      view.state.tr.setMeta(headingFoldingKey, {
        type: "fold",
        pos: positions[1],
      }),
    );
    ...
    // to the section.
    const fromHeading = findDraggableBlock(
      view.state.doc,
      positions[1] + 2,
      undefined,
      view.state,
    )!;
    const fromBody = findDraggableBlock(
      view.state.doc,
      positions[2] + 2,
      undefined,
      view.state,
    )!;
    const fromHiddenTail = findDraggableBlock(
      view.state.doc,
      positions[3] + 2,
      undefined,
      view.state,
  ...
  it("a folded task item keeps its fold when moved inside the list", () => {
    ...
    view.dispatch(
      view.state.tr.setMeta(taskFoldingKey, { type: "fold", pos: parentFrom }),
    );
