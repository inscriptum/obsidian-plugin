    ...
    props: {
      decorations: ({ doc, selection }) => {
        ...
        if (title.content.size === 0) {
          ...
          decorations.push(
            Decoration.node(0, title.nodeSize, { ...notEmptyTitleAttributes }),
          );
        }

        const isContentConsistsJustOfMainFirstLine =
          doc.content.lastChild === mainFirstLine;

        if (
          isContentConsistsJustOfMainFirstLine &&
          mainFirstLine.content.size === 0
        ) {
          decorations.push(
            Decoration.node(
              title.nodeSize,
              title.nodeSize + mainFirstLine.nodeSize,
              {
