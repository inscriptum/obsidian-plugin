...
describe("nested taskList (subtasks)", () => {
  it("schema accepts a nested taskList inside a taskItem and round-trips through JSON", () => {
    ...
    expect(
      item?.content?.[1]?.content?.[0]?.content?.[0]?.content?.[0]?.text,
    ).toBe("child");
  });
  ...
  it("Shift-Tab lifts a subtask back to the top level", () => {
    ...
    expect(taskList?.content?.[1]?.content?.[0]?.content?.[0]?.text).toBe(
      "two",
    );
    expect(
      taskList?.content?.[0]?.content?.find((n) => n.type === "taskList"),
    ).toBeUndefined();
  });
