 ...
 * Set a code block
 */
function setHljsCodeBlock(
  this: AddCommandsThis,
  attributes?: { language: SupportedLanguage },
): Command {
  return (param) => {
    ...
    if (canWrapInCodeBlock) {
      ...
      let codeText = "";
      const parentNodePos = param.state.doc
        .resolve(param.state.selection.$anchor.pos)
        .before(1);
      ...
      const jsonContent: JSONContent =
        (newNode.content.toJSON() as JSONContent) || [];
