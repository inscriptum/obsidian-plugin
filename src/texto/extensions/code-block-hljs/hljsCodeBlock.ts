    ...
    return {
      ...
      "Mod-Enter": ({ editor }) => {
        ...
        editor.view.dispatch(
          tr
            ...
            ) // replace the next element (the splitted paragraph) into a new paragraph
      ...
      Backspace: ({ editor }) => {
        ...
        const previousParentNodePos = editor.state.doc
          ...
          .before(1);
        const previousParentNode = editor.state.doc.nodeAt(
          previousParentNodePos,
        );
  ...
  addInputRules() {
    return [
      nodeInputRule(
        backtickInputRegex,
        this.type,
        ({ groups }: ExtendedRegExpMatchArray) => groups ?? {},
      ),
      nodeInputRule(
        tildeInputRegex,
        this.type,
      ...
      new Plugin({
        props: {
          ...
          clipboardTextParser: (text, _$context, _plain) => {
            ...
            if (
              state.selection.$anchor.parent.type.name !== "hljsCodeBlockRow"
            ) {
          ...
          transformPastedHTML: (html, view) => {
            ...
            if (parentNode?.type === this.type) {
              ...
              if (
                pastedEl.body.childNodes.length !== 1 ||
                !isFirstChildCodeElement
              ) {
                // Convert all tags to a text with breaks, except "span", "a" and Presentation tags
                pastedEl.body
                  .querySelectorAll(
                    "*:not(span,a,em,strong,i,b,u,s,sup,sub,tt)",
                  )
                  .forEach((el) => {
                    el.replaceWith(`${el.textContent}\n`);
                  });

                return generateCodeBlockDomElement(
                  pastedEl.body.textContent ?? "",
                ).innerHTML;
              }
            }
  ...
  addNodeView() {
    ...
    return ({ editor, node, getPos }) => {
      ...
      const language = findLanguageByCssClass(
        (node.attrs as HljsCodeBlockAttrs).class,
      ...
      if (isFunction(getPos)) {
        ...
        container.props.onChange = onChangeLang(
          pos,
          editor,
          this.options,
          domCodeEl,
...
function onChangeLang(
  pos: number,
  editor: Editor,
  options: HljsCodeBlockOptions,
  domCodeEl: HTMLElement,
) {
