...
const isFragment = (
  nodeOrFragment: ProseMirrorNode | Fragment,
): nodeOrFragment is Fragment => {
  ...
  return ({ tr, dispatch, editor }) => {
    if (dispatch) {
      ...
      if (from === to && isOnlyBlockContent) {
        ...
        const isCodeParent =
          findParentNodeClosestToPos(pos, (node) => !!node.type.spec.code) !=
          null;
        const isEmptyTextBlock =
          pos.parent.isTextblock && !isCodeParent && !pos.parent.childCount;
      ...
      if (options.updateSelection) {
        ...
        if (
          (lastStep instanceof ReplaceStep ||
            lastStep instanceof ReplaceAroundStep) &&
          !isFragment(content) &&
