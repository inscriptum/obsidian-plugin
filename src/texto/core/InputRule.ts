...
function run(config: {
  ...
  const isCodeParent =
    findParentNodeClosestToPos($from, (node) => !!node.type.spec.code) != null;
...
export function inputRulesPlugin(props: {
  editor: Editor;
  rules: InputRule[];
}): Plugin<InputRuleState | null> {
  const { editor, rules } = props;
  const plugin: Plugin<InputRuleState | null> =
    new Plugin<InputRuleState | null>({
      state: {
        init() {
          return null;
        },
