...
export class Editor extends EventEmitter<EditorEvents> {
  ...
  constructor(options: Partial<EditorOptions> = {}) {
    ...
    this.on("transaction", ({ transaction }) => {
      ...
      const error: unknown[] = Array.isArray(errorMeta)
        ? errorMeta
        : [errorMeta];

      let textoErrorCode: TextoErrorType[TextoErrorTypeKey] =
        TEXTO_ERROR.TRANSACTION_ERROR;

      const collaborationPluginKey = (
        this.extensionStorage.collaboration as
      ...
      if (
        ...
        (
          transaction.getMeta(collaborationPluginKey) as
  ...
  public unregisterPlugin(nameOrPluginKey: string | PluginKey): void {
    ...
    const name =
      typeof nameOrPluginKey === "string"
        ? `${nameOrPluginKey}$`
        : (nameOrPluginKey as PluginKey & { key: string }).key;

    const state = this.state.reconfigure({
      plugins: this.state.plugins.filter(
        (plugin) => !(plugin as PluginWithKey).key.startsWith(name),
      ),
    });
  ...
  private dispatchTransaction(transaction: Transaction): void {
    ...
    const focus = transaction.getMeta("focus") as
      | { event: FocusEvent }
      | undefined;
    const blur = transaction.getMeta("blur") as
      | { event: FocusEvent }
      | undefined;
