	...
	return matches.map((pasteRuleMatch) => {
		...
		if (pasteRuleMatch.replaceWith) {
			if (!pasteRuleMatch.text.includes(pasteRuleMatch.replaceWith)) {
				console.warn(
					'[TEXTO WARN]: "pasteRuleMatch.replaceWith" must be part of "pasteRuleMatch.text".',
				);
			}
...
function run(config: {
	...
	state.doc.nodesBetween(from, to, (node, pos) => {
		const resPos = state.tr.doc.resolve(pos);
		const isCodeParent = findParentNodeClosestToPos(resPos, (node) => !!node.type.spec.code) != null;
		if (!node.isTextblock || isCodeParent) {
			return;
		}
